...
class PhotoTest extends WebTestCase
{
    ...
    function testIndex()
    {
        ...
        $this->click("css=a[href='/photogallery/uploads/IMG_1152.jpg']");
    ...
    function testAdmin()
    {
        ...
        $this->click("css=a[href='/photogallery/photo/admin?Photo_sort=caption']");
        $this->waitForEval("window.document.querySelectorAll('div.keys span')[0].textContent", "3"); //storeText("css=div.keys");

        $this->clickAndWait("css=a[href='/photogallery/photo/update/3']");
        $this->type('name=Photo[sort_order]', 'xxx');
        $this->clickAndWait("//input[@value='Save']");
        $this->verifyTextPresent('Sort Order must be a number.');
